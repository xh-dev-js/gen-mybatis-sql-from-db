...
class OracleDialect {
  ...
  mybatisToOracleSql = (statement) => {
    ...
    const placeHolders = function(stmt) {
  ...
  OracleSqlToMybatis = (statement) => {
    ...
    return [statement].concat(placeHolders)
      .reduce((stmt, placeHolder) =>
        stmt.replace(new RegExp(`[ ]*defined[ ]+${placeHolder}[ ]*=[ ]*[^ ;\n]+[ ]*`, 'g'), ''),
      )
      .replace(/&([^ \n;$]+)/g, '#{$1}')
      .replace(/^[\n ]+/g, '')
      .replace(/[\n ]+$/g, '')
